...
def features_selection(dataset_scaled, features_names, labels, option):
    if option == 1:
        ...
        features_reduce = features_importance[ind[0:39]]  # ESCOLHI POR EXEMPLO AS 40 MELHORES FEATURES
...
def roc_auc(dataset, label, features_names):
    scores = np.empty(0)

    for i in range(0, len(dataset[0])):
        fpr, tpr, thresholds = metrics.roc_curve(label, dataset[:, i], pos_label=1)
        scores = np.append(scores, metrics.roc_auc_score(label, dataset[:, i]))

    ind = np.argsort(scores)[::-1]
    print(scores[np.where(scores > 0.9)].shape)
    features_sort = dataset[ind[:]]
    features_name_sort = features_names[ind[:]]
    return features_sort, features_name_sort

features_sort, features_name_sort = roc_auc(dataset_scaled,labels_bin,features_names)